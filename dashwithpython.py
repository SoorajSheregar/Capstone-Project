...
def update_pie_chart(site):
    if site == 'ALL':
        ...
        fig = px.pie(data, values='counts', names='class', title='Total Success Launches for Site '+site)
    return fig
# TASK 4:
...
def update_graph(site_dropdown, payload_slider):
    if site_dropdown == 'ALL':
        ...
        fig.update_layout(title='Payload vs. Outcome for All Sites',
        ...
        fig = px.scatter(filtered_df, x="Payload Mass (kg)", y="class", color="Booster Version Category")
        fig.update_layout(title='Payload vs. Outcome for {}'.format(site_dropdown),
                          xaxis_title='Payload Mass (kg)',
                          yaxis_title='Class')
        return fig
